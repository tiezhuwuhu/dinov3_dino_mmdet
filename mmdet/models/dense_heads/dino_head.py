...
class DINOHead(DeformableDETRHead):
    ...
    def __init__(
            self,
            *args,
            point_euclidean_weight: float = 0.0,
            **kwargs):
        """Initialize Point-DINO head.

        Args:
            point_euclidean_weight (float):
                Weight lambda for pixel-space Euclidean point loss.
                Default 0.0 means the Euclidean loss is disabled.
        """
    ...
    def _loss_point_euclidean(
        ...
        if point_preds.dim() == 3:
            ...
            point_preds = point_preds.reshape(
                num_imgs, num_queries, 2)
        else:
            raise ValueError(
                f'Unexpected point_preds shape: '
                f'{tuple(point_preds.shape)}')
    
        point_targets = point_targets.reshape(
            num_imgs, num_queries, 2)
    
        point_weights = point_weights.reshape(
            num_imgs, num_queries, 2)
        ...
        # Normalized coordinate error -> pixel error.
        delta_pixel = (
            point_preds - point_targets
        ) * point_scales
    
        eps = 1e-6
    
        distance = torch.sqrt(
            (delta_pixel ** 2).sum(dim=-1)
            + eps)
    
        # Only positive matched queries participate.
        positive_weights = point_weights[..., 0]
    
        loss = (
            distance * positive_weights
        ).sum() / max(float(avg_factor), 1.0)
    
        # Normalize by the 8-pixel evaluation scale.
        loss = loss / 8.0
    ...
    def loss_by_feat(
        ...
        for num_dec_layer, (
            ...
            loss_dict[
                f'd{num_dec_layer}.loss_point_euclidean'
            ] = loss_point_euclidean_i
        ...
        if all_layers_denoising_cls_scores is not None:
            ...
            loss_dict['dn_loss_point_euclidean'] = dn_losses_point_euclidean[-1]
            ...
            for num_dec_layer, (
                ...
                loss_dict[f'd{num_dec_layer}.dn_loss_point_euclidean'] = loss_point_euclidean_i
    ...
    def loss_dn(
            ...
            batch_gt_instances: InstanceList,
            batch_img_metas: List[dict],
            dn_meta: Dict[str, int]) -> Tuple[List[Tensor], List[Tensor], List[Tensor]]:
    ...
    def _loss_dn_single(
        ...
        # ---------------------------------------------------------
        # Point Euclidean loss
        # dn_point_preds shape: [B, N_dn, 2]
        # ---------------------------------------------------------
        loss_point_euclidean = self._loss_point_euclidean(
        ...
        # ---------------------------------------------------------
        # Point L1 loss
        # self.loss_bbox expects flattened predictions.
        # ---------------------------------------------------------
