...
from ..dense_heads.point_fidt_aux_head import PointFIDTAuxHead
...
class DINO(DeformableDETR):
    ...
    Args:
        ...
        point_fidt_head (dict, optional): Training-only spatial FIDT branch.
            Disabled by default; enabled=False or fidt_loss_weight=0 also
            skips construction and keeps the Stage 3 parameter set.
    """

    def __init__(self, *args, dn_cfg: OptConfigType = None,
                 use_dn: bool = True,
                 point_fidt_head: OptConfigType = None, **kwargs) -> None:
        ...
        if point_fidt_head is not None:
            ...
            if enabled and fidt_cfg.get('fidt_loss_weight', 0.0) != 0:
                self.point_fidt_head = PointFIDTAuxHead(**fidt_cfg)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # Share the highest-resolution transformer input without detaching,
        # resampling, or otherwise changing the original transformer inputs.
        shared_feature = max(img_feats, key=lambda feat: feat.shape[-2]
                             * feat.shape[-1])
        losses.update(self.point_fidt_head.loss(
            shared_feature, batch_data_samples, batch_inputs.shape[-2:]))
        return losses
